...
The Weekly Resume MCP server can manage job applications as well as resumes. When the user wants to track applications through an agent, use the application tools instead of asking them to open the web UI.
...
Output must conform to the Weekly Resume schema. See [references/schema.md](references/schema.md) for the complete schema structure.
...
When generating the resume, output a complete JSON object that conforms to the Weekly Resume schema. The user can then import this JSON directly into Weekly Resume.
