...
description: "The Weekly Resume JSON Schema, published at /schema.json: what it covers, how to validate resume data against it, and how to build on it."
---
...
Weekly Resume aims to be the gold standard in JSON resume schemas: one documented, strictly typed schema that any service can adopt so resumes move between tools without loss.

Imagine:

- Exporting your resume from Weekly Resume as a JSON file
...
The Weekly Resume JSON Schema is publicly available at:

<Info>`/schema.json`（由本实例提供）</Info>

The endpoint returns a complete JSON Schema in the draft 2020-12 dialect. It describes the structure of a Weekly Resume document and covers:
...
- **Job portals** can accept Weekly Resume JSON files directly
...
- **Canonical contract**: one public schema endpoint tracks the current Weekly Resume data model

The schema is open and free to use. Fetch it programmatically, or reference it by URL from your own systems.

## Complete JSON Schema

The full JSON Schema for Weekly Resume follows. You can also fetch the latest version directly from `/schema.json`（由本实例提供）.
