...
describe("BrandIcon", () => {
	it("renders two img elements (light + dark variants)", () => {
		render(<BrandIcon />);
		const imgs = screen.getAllByAltText("Weekly Resume");
		expect(imgs).toHaveLength(2);
	});

	it("uses the current icon assets for the legacy default logo variant", () => {
		render(<BrandIcon />);
		const imgs = screen.getAllByAltText("Weekly Resume");
	...
	it("hides dark variant by default (light mode); dark mode reveals it via dark:block", () => {
		render(<BrandIcon />);
		const imgs = screen.getAllByAltText("Weekly Resume");
		const darkImg = imgs.find((img) => img.getAttribute("src") === "/icon/dark.svg");
