...
describe("buildMcpServerCard", () => {
	...
	it("identifies the server without the upstream hosted URL", () => {
		expect(card.serverInfo.name).toBe("weekly-resume");
		expect(card.serverInfo.title).toBe("Weekly Resume");
		expect(card.serverInfo.websiteUrl).toBeUndefined();
	});

	it("omits hosted icons until a website URL is provided", () => {
		expect(card.serverInfo.icons).toEqual([]);
		const hosted = buildMcpServerCard("1.2.3");
		expect(hosted.serverInfo.icons).toEqual([]);
	});
