...
Weekly Resume is a monochrome, content-first design system for creating and managing resumes. The visual identity prioritizes readability and unobtrusiveness — the user's resume content is always the hero, never the chrome around it.
...
Template cards use localized, descriptive names that explain the layout and intended scenario. Legacy internal template identifiers remain in code and stored data for compatibility; never expose those identifiers as user-facing labels.
