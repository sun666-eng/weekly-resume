...
describe("PdfViewer", () => {
	...
	it("renders exposed semantic source through the shared PDF entrypoint", async () => {
		...
		render(<PdfViewer data={semanticData} publicResume={{ username: "demo-user", slug: "sample" }} />);
	...
	it("uses the authorized PDF fallback after browser generation rejects", async () => {
		...
		render(<PdfViewer data={sampleResumeData} publicResume={{ username: "demo-user", slug: "sample" }} />);

		await waitFor(() => expect(pdfViewerMock.fetch).toHaveBeenCalledTimes(1));
		expect(pdfViewerMock.fetch).toHaveBeenCalledWith("/api/resumes/demo-user/sample/pdf", { credentials: "include" });
	});
});
