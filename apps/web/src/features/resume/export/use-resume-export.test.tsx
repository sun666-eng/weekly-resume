	...
	it("downloads the authorized server blob after public browser rendering rejects", async () => {
		...
		const { result } = renderHook(() =>
			useResumeExport(
				...
				{
					publicResumePdf: {
						publicResume: { username: "demo-user", slug: "sample" },
					},
				},
			),
		);
		...
		expect(mocks.recordDownload).toHaveBeenCalledExactlyOnceWith({ username: "demo-user", slug: "sample" });
	});

	const publicResume = { username: "demo-user", slug: "sample" };
