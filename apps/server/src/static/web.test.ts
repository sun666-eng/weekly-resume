...
const mocks = vi.hoisted(() => ({
	env: { APP_URL: "https://weekly-resume.example.com", ROOT_RESUME_ID: undefined as string | undefined },
...
describe("web app fallback classification", () => {
	...
	it("injects canonical metadata and structured data into tracking-parameter root requests only", async () => {
		vi.mocked(fs.readFile).mockResolvedValue(`
			...
			<html>
				<head>
					<title>Weekly Resume — 免费开源的简历制作工具</title>
					<meta
						name="description"
						content="免费开源的简历制作工具。轻松创建、编辑和分享专业简历，无广告、无付费墙。"
					>
				</head>
		...
		expect(html).toContain('<link rel="canonical" href="https://weekly-resume.example.com/">');
		expect(html).toContain('<link rel="preload" href="/videos/timelapse-v1.webp" as="image" fetchpriority="high">');
		expect(html).toContain('<meta property="og:url" content="https://weekly-resume.example.com/">');
		expect(html).toContain(
			'<meta property="og:image" content="https://weekly-resume.example.com/opengraph/banner.jpg">',
		);
		expect(html).toContain('id="weekly-resume-structured-data"');
		expect(html).toContain('"@type":["SoftwareApplication","WebApplication"]');
		expect(html).toContain('"url":"https://weekly-resume.example.com/"');
	...
	describe("the ATS checker page", () => {
		const shell = `<html><head><title>Weekly Resume — 免费开源的简历制作工具</title><meta name="description" content="Marketing copy."></head><body></body></html>`;
		...
		it("replaces the shell metadata with the checker's own", async () => {
			...
			expect(html).toContain("<title>ATS Checker - Weekly Resume</title>");
			expect(html).toContain('<link rel="canonical" href="https://weekly-resume.example.com/ats-checker">');
			expect(html).toContain('<meta property="og:url" content="https://weekly-resume.example.com/ats-checker">');
			expect(html).toContain(
				'<meta property="og:image" content="https://weekly-resume.example.com/opengraph/ats-checker.png">',
			);
	...
	describe("public resume social cards", () => {
		...
		it("injects resume-specific social metadata and replaces the shell title", async () => {
			...
			expect(html).toContain("<title>Jane Doe - Weekly Resume</title>");
			expect(html).toContain('<meta name="description" content="Builds resilient distributed systems.">');
			expect(html).not.toContain("Marketing copy.");
			expect(html).toContain('<link rel="canonical" href="https://weekly-resume.example.com/jane/resume">');
			...
			expect(html).toContain(
				'<meta name="twitter:image" content="https://weekly-resume.example.com/opengraph/banner.jpg">',
			);
		});
...
describe("configured root shell", () => {
	...
	it("uses configured canonical root without leaking ID or marketing metadata", async () => {
		...
		expect(html).toContain('<link rel="canonical" href="https://weekly-resume.example.com/" data-root-resume-shell>');
